<template>
    <div class="validator">
        ...
        <a href="javascript:;" class="v-clear iconfont" tabindex="-1" @click="clearInput" v-show="showClearIcon && showClear && !isempty"></a>
        <span class="v-error iconfont" v-if="showErrorIcon" v-show="(!!regex || !!min || !!max || required) && iserror && initError"></span>
        <span class="v-warn iconfont" v-if="showRequiredIcon && showErrorIcon" v-show="(required || (!!min && min > 0)) && isempty && showWarn"></span>
        <span class="v-success iconfont" v-if="showSuccessIcon" v-show="(!!regex || !!min || !!max || required) && !iserror && currentValue != ''"></span>
        <a href="javascript:;" v-if="type == 'password'" class="v-password iconfont" :class="showPwd ? 'v-password-open' : ''" tabindex="-1" @click.stop="showPwd = !showPwd"></a>
    </div>
</template>
...
<script>
    export default {
        ...
        props: {
            ...
            showClearIcon: {    // 是否显示清空内容图标
                type: Boolean,
                default: true
            },
            showErrorIcon: {    // 是否显示错误图标
                type: Boolean,
                default: true
            },
            showSuccessIcon: {  // 是否显示正确图标
                type: Boolean,
                default: true
            },
            showRequiredIcon: { // 是否显示必填提示图标
                type: Boolean,
                default: true
            },
            required: {   // 是否必填
        ...
        methods: {
            ...
            setError (error, code) {
                this.errorMsg = this.errTip + error;
                this.errorCode = code;
                this.valid = false;
            },
            trim (str, char) {
