        ...
        <scroll :on-refresh="onRefresh" :on-infinite="onInfinite"  v-if="items.length">
            <div class="bgee clearfix goodsList">
                <div class="clearfix bgf mb10" v-for="(item,index) in items">
                    <!-- item.Planid == 102全球购 -->
                    <router-link class="goodsItem" :to="(item.Planid == 102? '/gobal': '')+'/goods?planid=' + item.Planid + '&tid='+ item.Tid" if="item.Planid != 102">
                        <img class="fl" v-lazy="'' + item.Img" width="124" height="124" v-if="item.Planid != 102">
                        <img class="fl" v-lazy="'http:// msyc-img.oss-cn-shenzhen.aliyuncs.com' + item.Img" width="124" height="124" v-if="item.Planid == 102">
                        <div class="con ofh">
                            <h3 class="tit">
                                {{item.Cname}}
                            </h3>
