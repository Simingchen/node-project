    ...
    <page-content>
        <ul class="receiver-msg">
            ...
            <li class="items-line bgf bor-bottom">
                ...
                <input id="receiver" type="text" placeholder="身份证,作为全球购身份验证(选填)" maxlength="25" v-model.trim="items.Cardid">
            </li>
            <li class="items-line bgf bor-bottom">
                <label for="phone">手机号</label>
                <input id="phone" type="text" placeholder="手机号(必填)" maxlength="11" v-model.trim="items.Mobile" pattern="[0-9]*">
            </li>
            <li class="items-line bgf bor-bottom">
                <label for="phone">固定电话号</label>
                <input id="phone" type="text" placeholder="固定电话号(选填)" maxlength="20" v-model.trim="items.Tel">
            </li>
            <li class="items-line bgf bor-bottom">
            ...
            <li class="items-line bgf bor-bottom">
                ...
                <input id="address" type="text" placeholder="如街道,楼层,门牌号等(必填)" maxlength="20" v-model.trim="items.Addr">
            </li>
            <li class="items-line bgf bor-bottom">
                <label for="zipCode">邮政编码</label>
                <input id="zipCode" type="text" placeholder="邮政编码(选填)" maxlength="10" v-model.trim="items.Postcode" pattern="[0-9]*">
            </li>
            <li class="items-line bgf bor-bottom">
                <label for="zipCode">邮件</label>
                <input id="zipCode" type="email" placeholder="邮件(选填)" maxlength="10" v-model.trim="items.Email">
            </li>
            <li class="bgf">
    ...
    export default {
        ...
        methods: {
            ...
            saveReceiverMsg: function () {
                var reg;
                var isTrue
                if (!this.items.Shouman) {
                    this.verifyError("请输入名字");
                    return false;
                }
                if (this.items.Cardid) {
                    reg = /(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/;
                    isTrue = reg.test(this.items.identity);
                    if (!isTrue) {
                ...
                if (this.items.Tel) {
                    reg = /\d{3}-\d{8}|\d{4}-\d{7}/;
                    isTrue = reg.test(this.items.Tel);
                    if (!isTrue) {
                ...
                if (this.items.Postcode) {
                    reg = /^[1-9][0-9]{5}$/
                    isTrue = reg.test(this.items.zipCode);
                    if (!isTrue) {
