                    ...
                    <div class="goods-li ofh" v-for="(item,index) in items">
                        <ul v-for="(subItem,index) in item.subdanlist">
                            ...
                            <div class="state-li">
                                ...
                                <p class="c58" v-if="item.danstate == 1000">未支付</p>
                                <p class="c58" v-if="item.danstate == 2000">已收款</p>
                                <p class="c58" v-if="item.danstate == 2500">已同意发货</p>
                                <p class="c58" v-if="item.danstate == 3000">已发货</p>
                                <p class="c58" v-if="item.danstate == 6000">已收货</p>
                                <p class="c58" v-if="item.danstate == -1">已取消</p>
                            </div>
                            <li class="item clearfix" v-for="(order,index2) in subItem.dantaillist">
                                <router-link :to="'/orderDetail?code=' + item.danid" >
                                <img class="img fl" :src="order.img | formatImg('','')" width="90" height="90">
                        ...
                        <div class="mb10 clearfix">
                            ...
                            <span class="delete-btn" @click="payPop(item)" v-if="item.danstate == 1100 || item.danstate == 1000">在线支付</span>
...
// 已处理 = 9
        ...
        methods:{
            ...
            payOrder:function(){
                ...
                this.$http.post(url,{
                    danid: this.curItem.danid,
                    paypass:this.$children[2].mutableInput,   //第三个子组件下的值
                }).then(function(data){
                    var oData = data.body;
                    ...
                    if(oResult == 8){   //购买不足积分，唤起微信支付
                        var oParame   = oData.Data,
                            ...
                            nonce_str = oParame.nonceStr,
                            prepay_id = oParame.package,
        ...
        filters:{
            ...
            //判断是否是全球购资源
            // formatImg:function(value,id){
            //     if(id == 102){
            //         return "http://msyc-img.oss-cn-shenzhen.aliyuncs.com"+ value;
            //     }else{
            //         return "http://www.shihuamall.com/Thumbnail/BusGoodsList"+ value;
            //     }
            // },
        }
    }
