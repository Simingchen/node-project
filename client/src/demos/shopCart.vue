...
<div class="pageCon shopCart-page">
    ...
    <div v-if="!isLogin">
        ...
        <div class="edit-bar" v-if="cartItems.length">
            ...
            <div class="fl" @touchstart="allSelect(isAllSelect)" v-if="!isEditStatus">
        ...
        <div class="page-content" v-if="cartItems.length">
            <div class="listCon">
                ...
                <div class="item-cart bor-bottom bgf" v-for="(item,index) in cartItems">
                    <!-- 非编辑状态 -->
                    <div class="check-container" @touchstart="selectProduct(item)" v-if="!isEditStatus">
                        <span class="form-checkbox" :class="{checked : item.checked}"></span>
                    </div>
                    <!-- 编辑状态 -->
                    <div class="check-container" @touchstart="selectDeleted(item)" v-if="isEditStatus">
                        <span class="form-checkbox" :class="{checked : item.deleted}"></span>
                    </div>
                    <div class="name-card clearfix">
                        <router-link :to="'/goods?planid=' + item.Planid + '&tid='+ item.Tid">
                            <img class="thumb" v-lazy="'http://www.shihuamall.com/Thumbnail/BusGoodsList'+item.Thumbnail">
                        </router-link>
                        <div class="detail">
                            <h3 class="title">
                                <router-link :to="'/goods?planid=' + item.Planid + '&tid='+ item.Tid" v-if="!isEditStatus">
                                    {{item.Hname}}
                                </router-link>
                            </h3>
            ...
            <div class="subfooter" v-if="isEditStatus">
                <div class="item clearfix">
                    <div class="fl" @touchstart="selectAllDeleted(isAllSelectDeleted)">
                        <span class="form-checkbox" :class="{checked : allSelectDeleted}"></span>
                        <span>全选</span>
                    </div>
                </div>
            ...
            <div class="subfooter" v-if="!isEditStatus">
                <div class="item clearfix">
                    <div class="fl" @touchstart="allSelect(isAllSelect)">
                        <span class="form-checkbox" :class="{checked : allSelectBtn}"></span>
                        <span>全选</span>
                    </div>
        ...
        methods:{
            ...
            DeleteOk:function(m) {
                ...
                if(this.isMultProduct == 1){
                    var url = apiList.data.DeleteFromShoppingCar;
                    var code = "";
                    this.cartItems.forEach((item,index)=>{
                        if(item.deleted){
                            //计算商品数量
                    ...
                    this.$http.post(url,{
                        ...
                        if(data.body.Result == 1){
                            ...
                            this.isEmpty = this.cartItems.length <=0 ? true :false;
                        }else{  //失败
                            this.tipsContent = data.body.Message;
            ...
            goPay:function(){
                ...
                if(this.isWeChatBrowser){
                    ...
                    this.$http.post(url,{
                        ...
                        if(oData.Result == 1){
                            //总额大于积分需要唤起微信支付
                            if(this.totalMoney > oData.Accountlist[0].Fencur){
                                this.getWeChat('/order?type=1&goods='+selectGoods);
                            }else{
